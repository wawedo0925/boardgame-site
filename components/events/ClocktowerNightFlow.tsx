...
export default function ClocktowerNightFlow({state,run,busy}:{state:LiveState;run:Run;busy:boolean}) {
  ...
  return <section className="space-y-4 rounded-3xl border border-violet-400/30 bg-violet-400/5 p-5">
    ...
    {room.phase==='NIGHT'&&<>
      ...
      {engine.night!==room.night?<p>밤 순서를 준비하고 있습니다.</p>:engine.finished?<p className="text-emerald-300">밤 시트 완료 · 낮 시작 버튼으로 사망을 공개하세요.</p>:<>
        ...
        {task&&current?.status==='SUBMITTED'&&<ProposalReview key={`${current.id}:${version}`} task={task} request={current} engine={engine} members={members} busy={busy} approve={(result,proposal)=>{const effects=approveEffects(task,current.targets,engine,members,proposal);return run('engine_approve',{version,request_id:current.id,result,...effects});}}/>}
...
function ProposalReview({task,request,engine,members,busy,approve}:{task:NightTask;request:LiveRequest;engine:NightEngine;members:LiveMember[];busy:boolean;approve:(result:string,p:ReturnType<typeof propose>)=>Promise<boolean>}){
  ...
  return <div className="space-y-4 rounded-2xl border border-white/15 bg-zinc-950 p-4">

...
import ClocktowerSeating, { ClocktowerName } from './ClocktowerSeating';
...
export default function ClocktowerLive({ eventId, expectedRoomId }: { eventId: string; expectedRoomId?: string }) {
  ...
  return <div className="clocktower-mobile mt-6 min-w-0">
    ...
    {hidden ? <div className="rounded-3xl border border-white/10 p-16 text-center text-zinc-500">화면을 가렸습니다.</div> : !state ? <p>진행방을 불러오는 중…</p> : !room ? <div className="rounded-3xl border border-white/10 p-6">
      ...
      {state.can_create && <><p className="my-4 text-sm leading-7 text-zinc-400">진행방을 만든 계정이 이야기꾼이 됩니다. 이야기꾼만 실제 역할과 전체 선택을 볼 수 있습니다. 일정 참가자를 자동으로 불러옵니다. 자리·역할·비밀 정보를 저장한 뒤 역할을 배포하고 전원 확인을 받으세요.</p><button disabled={busy} className={button} onClick={() => void run('create')}>내가 이야기꾼으로 진행방 만들기</button></>}
    </div> : <>
...
function Host({ state, run, busy, allowPopup, error }: { state: LiveState; run: Run; busy: boolean; allowPopup:boolean; error:string }) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap gap-3">{room.phase === 'ENDED' ? <button disabled={busy} className={button} onClick={() => { if (confirm('새 게임을 준비할까요? 이전 게임은 보관되고 화면은 새 게임으로 전환됩니다.')) void run('create'); }}>새 게임 준비</button> : <>
      <button disabled={busy || layoutEditing || rolesEditing || informationEditing || (room.phase==='SETUP' && (invalidSetup||invalidInformation||(room.roles_released&&members.some(m=>!m.role_confirmed)))) || (room.phase==='NIGHT'&&((state.mission_progress??[]).some(m=>m.completed<m.total)||pending.length>0||state.engine?.night!==room.night||!state.engine?.finished)) || (state.votes??[]).some(v=>v.status==='WAITING'||v.status==='RUNNING')} className={button} onClick={()=>{if(confirm(`${nextFlowLabel(state)} 단계로 넘어갈까요?${room.phase==='DAY'&&room.day_stage==='NOMINATIONS'?' 현재 단독 최다 처형 후보가 있으면 처형합니다.':''}`))void run('flow_next',{revision:room.flow_revision??0});}}>다음 페이즈 · {nextFlowLabel(state)}</button>
      <button disabled={busy} className={subtle} onClick={() => void phase('ENDED')}>게임 종료</button>
    </>}</div>
    {room.phase==='ENDED'&&<p className="rounded-xl bg-violet-400/10 p-4 text-sm">{room.result_round_id?'캐릭터·승리 진영과 플레이 이력이 자동 저장되었습니다. 일정에서 다시 저장할 필요가 없습니다.':room.winner?'이 게임은 자동 저장 적용 전 기록입니다. 일정 상세에서 결과를 저장해 주세요.':'승리 진영 없이 종료했습니다. 결과를 기록하려면 일정 상세에서 승리 진영을 선택해 주세요.'}</p>}
    ...
    {room.phase==='NIGHT'&&!!state.mission_progress?.length&&<section className="rounded-2xl border border-white/15 p-5"><details className="group"><summary className="flex cursor-pointer items-center justify-between font-bold"><span>공통 미션 진행</span><span className="text-sm text-violet-200"><span className="group-open:hidden">더보기</span><span className="hidden group-open:inline">접기</span></span></summary><p className="my-3 text-sm text-zinc-400">도착한 미션을 모두 완료하면 낮으로 넘어갈 수 있습니다.</p><div className="space-y-2">{state.mission_progress.map(m=><div key={m.user_id} className="flex flex-wrap items-center justify-between gap-2"><span>{members.find(p=>p.user_id===m.user_id)?.name} · {m.completed}/{m.total}</span>{m.completed<m.total&&<button disabled={busy} className={subtle} onClick={()=>{if(confirm('휴대폰 사용이 어려운 참가자의 현재 미션을 현장에서 완료 처리할까요?'))void run('mission_offline',{user_id:m.user_id});}}>현장 완료 처리</button>}</div>)}</div></details></section>}
    ...
    <ClocktowerSeating vote={(state.votes??[]).find(v=>room.phase==='DAY'&&v.status==='RUNNING')} onRecordVote={(voteId,userId,yes)=>run('vote_record',{vote_id:voteId,user_id:userId,yes})} roomId={room.id} storyteller engine={state.engine} phase={room.phase} night={room.night} key={room.id + members.map(m => m.user_id).sort().join(',')} members={members} onEditingChange={setLayoutEditing} layout={room.seating_layout} revision={room.seating_revision} save={(layout, revision) => run('save_layout', { layout, revision })} editable={room.phase === 'SETUP'&&!room.roles_released} busy={busy || rolesEditing} myId={state.my_id} sync={() => void run('sync_participants')} swap={(a, b) => run('swap_seats', { first_user_id: a.user_id, second_user_id: b.user_id, first_seat: a.seat, second_seat: b.seat })} />
    ...
    {room.phase === 'NIGHT' && !state.engine?.night && <form ref={requestForm} className="scroll-mt-6 rounded-3xl border border-violet-400/30 p-5" onSubmit={async e => { e.preventDefault(); if (await run('request', { user_id: recipient, night: room.night, prompt, target_count: count, allow_self: self })) { setPrompt(''); setRecipient(''); } }}>
      ...
      <fieldset disabled={busy} className="space-y-4"><label className="block">받는 사람<select required value={recipient} onChange={e => { const m = members.find(m=>m.user_id===e.target.value); if(m) prepare(m); else setRecipient(''); }} className={`${field} mt-2`}><option value="">참가자 선택</option>{members.map(m=><option key={m.user_id} value={m.user_id}>{m.seat}. {m.name} · {m.shown_role || '역할 미배정'}</option>)}</select></label>

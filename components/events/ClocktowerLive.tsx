...
function Player({ state, run, busy, allowPopup, error }: { state: LiveState; run: Run; busy: boolean; allowPopup:boolean; error:string }) {
  ...
  return <div className="space-y-5">{state.room?.phase==='SETUP'&&state.room.roles_released&&!me?.role_confirmed&&<ClocktowerPopup title="내 역할을 확인해 주세요" busy={busy} onClose={()=>{}}><p className="my-6 text-center text-3xl font-bold">{me?.shown_role}</p><p>다른 사람에게 화면을 보여주지 마세요.</p>{error&&<p role="alert">{error}</p>}<button disabled={busy} className={button} onClick={()=>void run('role_ack')}>역할 확인 완료</button></ClocktowerPopup>}{state.room?.phase==='SETUP'&&me?.role_confirmed&&<p>역할 확인 완료 · 모두 확인하면 이야기꾼이 첫날 밤을 시작합니다.</p>}<div className="rounded-3xl border border-violet-400/30 p-6"><p className="text-zinc-400">내 역할</p><h2 className="mt-2 text-3xl font-bold text-violet-200">{me?.shown_role ?? '이야기꾼이 준비하고 있습니다'}</h2><p className="mt-3 text-sm text-zinc-400">{me?.seat}번 자리 · {me && <ClocktowerName member={me} />}</p></div>

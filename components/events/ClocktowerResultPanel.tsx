...
export default function ClocktowerResultPanel({eventId,title,participants,canManage,isClosed}:Props){
  ...
  return <section className="rounded-3xl border border-violet-400/25 bg-violet-400/[0.035] p-5 text-white sm:p-7">
    ...
    {!difficulty?<p className="mt-5 text-red-300">이벤트 제목에서 난이도를 확인할 수 없습니다.</p>:characters.length===0?<p className="mt-5 rounded-2xl border border-white/10 p-5 text-zinc-400">캐러셀 캐릭터 목록은 추후 업데이트됩니다.</p>:<>
      <div className="mt-6 grid gap-3 sm:grid-cols-2">{eligible.map(member=><label key={member.user_id} className="rounded-2xl border border-white/10 bg-black/20 p-4"><span className="font-bold">{nameOf(member)}</span><select disabled={isClosed} value={characterByUser[member.user_id]??""} onChange={e=>setCharacterByUser(current=>({...current,[member.user_id]:e.target.value}))} className="mt-3 h-12 w-full rounded-xl border border-white/10 bg-zinc-900 px-3"><option value="">캐릭터 선택</option>{(["주민","외지인","하수인","악마"] as const).map(type=><optgroup key={type} label={type}>{characters.filter(c=>c.type===type).map(c=><option key={c.name} value={c.name}>{c.name} · {c.faction}</option>)}</optgroup>)}</select></label>)}</div>

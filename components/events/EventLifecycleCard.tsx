  ...
  async function changeStatus(nextClosed: boolean) {
    ...
    try {
      ...
      const { error } = await supabase
        .from("events")
        .update({
          event_status: nextClosed ? "CLOSED" : "OPEN",
  ...
  return (
    ...
    >
      <div className="flex h-full flex-col justify-between gap-4">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="min-w-0">
            <p
              className={`text-sm font-semibold ${
                isClosed ? "text-emerald-300" : "text-amber-300"
              }`}
            >
              EVENT STATUS
            </p>
          ...
          {canManage && (
            <button
              ...
              className={`h-12 shrink-0 rounded-xl px-5 font-bold disabled:cursor-not-allowed disabled:opacity-50 ${
                isClosed
        ...
        {isClosed && (
          <p className="rounded-xl bg-black/20 px-4 py-3 text-sm leading-6 text-emerald-200">
            마감된 이벤트는 기록을 볼 수만 있습니다. 수정하려면 생성자가
            이벤트를 다시 열어야 합니다.
          </p>
        )}
      </div>

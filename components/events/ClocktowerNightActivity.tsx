...
export default function ClocktowerNightActivity({id,prompt,message,members,count,busy,seconds,onConfirm,children}:{
  ...
  return <div className="space-y-4">
    ...
    <p className="text-sm text-violet-200" role="status">{remaining>0?count>0?`${remaining}초 뒤 멤버를 선택할 수 있습니다.`:`${remaining}초 뒤 확인 버튼이 활성화됩니다.`:count>0&&picked.length!==count?'멤버를 선택해 주세요.':'확인 버튼을 눌러 주세요.'}</p>

...
export default function ClocktowerPopup({title,subtitle,children,onClose,busy=false}:{title:string;subtitle?:string;children:ReactNode;onClose:()=>void;busy?:boolean}) {
  ...
  useEffect(()=>{
    ...
    const resize=()=>{
      if(!dialog)return;
      dialog.style.setProperty('--popup-height',`${visual?.height??window.innerHeight}px`);
      dialog.style.setProperty('--popup-top',`${visual?.offsetTop??0}px`);
    };
    resize();
    visual?.addEventListener('resize',resize);
    visual?.addEventListener('scroll',resize);
    window.addEventListener('resize',resize);
    return ()=>{visual?.removeEventListener('resize',resize);visual?.removeEventListener('scroll',resize);window.removeEventListener('resize',resize);dialog?.close();document.body.style.overflow=overflow;if(previous?.isConnected)previous.focus();};
  },[]);
  return <dialog ref={ref} aria-labelledby={id} onCancel={e=>{e.preventDefault();if(!busy)onClose();}} className="clocktower-mobile clocktower-popup m-auto max-h-[90dvh] w-[calc(100%-2rem)] max-w-2xl overflow-hidden rounded-3xl border border-violet-300/25 bg-zinc-950 p-0 text-white shadow-2xl backdrop:bg-black/80 backdrop:backdrop-blur-sm">
    <div className="clocktower-popup-frame flex max-h-[90dvh] flex-col"><header className="flex shrink-0 items-start justify-between gap-4 border-b border-white/10 px-5 py-4"><div>{subtitle&&<p className="mb-1 text-xs tracking-wider text-violet-300">{subtitle}</p>}<h2 ref={titleRef} tabIndex={-1} id={id} className="text-2xl font-bold outline-none">{title}</h2></div><button disabled={busy} aria-label="팝업 잠시 닫기" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-full bg-white/5 text-2xl hover:bg-white/10 disabled:opacity-40" onClick={onClose}>×</button></header><div className="min-h-0 overflow-y-auto overscroll-contain p-5">{children}</div></div>
  </dialog>;
}

...
import DeathStationResultDialog from "./DeathStationResultDialog";
import WolfStreetResultDialog from "./WolfStreetResultDialog";

import { scoreRanks, scoreRankLabel } from "@/lib/score-rank";
...
export default function RoundResultDialog(props:Props) { return isDeathStation(props.gameName)?<DeathStationResultDialog round={props.round} onClose={props.onClose} onSaved={props.onSaved}/>:props.cooperative?<CooperativeResultDialog round={props.round} onClose={props.onClose} onSaved={props.onSaved}/>:isWolfStreet(props.gameName)?<WolfStreetResultDialog round={props.round} onClose={props.onClose} onSaved={props.onSaved}/>:<DefaultRoundResultDialog {...props}/>; }
function DefaultRoundResultDialog({ round, resultType, gameName, onClose, onSaved }: Props) {
  ...
  let autoRanks: ReturnType<typeof scoreRanks> = [];
  if (!teamScore) { try { autoRanks = scoreRanks(activePlayers.map(p=>({userId:p.user_id,score:(scores[p.user_id]??"").trim()===""?null:Number(scores[p.user_id]),rank:ranks[p.user_id]??null}))); } catch {} }
  const hasScores = activePlayers.some(p=>(scores[p.user_id]??"").trim()!=="");
  ...
  return <div className="fixed inset-0 z-[100] flex items-end bg-black/70 sm:items-center sm:justify-center" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="max-h-[92dvh] w-full overflow-y-auto rounded-t-3xl border border-white/10 bg-zinc-950 p-5 text-white shadow-2xl sm:max-w-lg sm:rounded-3xl sm:p-7">
      ...
      <div className="flex items-start justify-between gap-4"><div><p className="text-sm text-amber-300">{round.round_number}판</p><h2 className="mt-1 text-xl font-bold">{teamScore ? "티츄 팀 점수 입력" : "점수/등수 입력"}</h2></div><button onClick={onClose} className="min-h-11 min-w-11 rounded-full bg-white/5 text-xl" aria-label="닫기">×</button></div>
      ...
      <div className="mt-6 space-y-3">
        {round.players.map((player) => <div key={player.user_id} className="rounded-2xl border border-white/10 bg-white/[0.03] p-4">
          ...
          {gmByUser[player.user_id] ? <p className="rounded-xl bg-violet-400/10 p-4 text-sm text-violet-200">GM 진행으로 기록합니다. 점수·등수는 저장하지 않습니다.</p> : <div className="space-y-3"><div className="text-sm"><p>{teamScore?'점수':'점수 (등수만 기록하면 비워 두세요)'}</p><SignedScoreInput name={playerName(player)} disabled={busy} value={scores[player.user_id] ?? ""} onChange={value=>setScores(current=>({...current,[player.user_id]:value}))}/></div>{!teamScore&&(hasScores?<p className="text-sm text-amber-300">{autoRanks.length?scoreRankLabel({score:null,rank:autoRanks.find(p=>p.userId===player.user_id)?.rank??null},autoRanks):'모든 점수를 입력하면 등수가 자동 계산됩니다.'}</p>:<label className="block text-sm">등수<select disabled={busy} value={ranks[player.user_id]??''} onChange={event=>setRanks(current=>({...current,[player.user_id]:event.target.value?Number(event.target.value):null}))} className="mt-2 h-12 w-full rounded-xl border border-white/10 bg-zinc-900 px-3"><option value="">등수 선택</option>{activePlayers.map((_,index)=><option key={index+1} value={index+1}>{index+1}등</option>)}</select><span className="mt-1 block text-xs text-zinc-400">공동 등수는 같은 등수를 선택하세요.</span></label>)}</div>}
        </div>)}
      </div>

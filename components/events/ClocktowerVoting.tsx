...
export default function ClocktowerVoting({state,run,busy,error}:{state:LiveState;run:(action:string,data?:Record<string,unknown>)=>Promise<boolean>;busy:boolean;error:string}){
 ...
 return <section className="space-y-4 rounded-3xl border border-violet-400/30 p-5">
  ...
  {active&&<div className="space-y-3 rounded-2xl bg-violet-400/10 p-4"><p className="text-lg">{name(active.nominator)} → <strong>{name(active.nominee)}</strong> 지목</p>
   {active.status==='WAITING'?<><p>변론을 마치면 이야기꾼이 투표를 시작합니다.</p><p className="text-sm text-zinc-400">순서: {[...members].sort((a,b)=>{const seat=members.find(m=>m.user_id===active.nominee)?.seat??0;return (a.seat>seat?0:1)-(b.seat>seat?0:1)||a.seat-b.seat;}).map(m=>m.name).join(' → ')}</p>{state.is_host&&<button disabled={busy} className={button} onClick={()=>void (async()=>{setButlerChecked(false);await run('vote_start',{vote_id:active.id});})()}>투표 시작 · 현장 수동 집계</button>}</>:<div className="space-y-3"><p>이야기꾼이 손 든 사람을 확인하고 집계하고 있습니다.</p>{state.is_host&&<><div className="grid grid-cols-2 gap-2 sm:grid-cols-3">{active.voter_order.map(id=>{const m=members.find(x=>x.user_id===id);const yes=active.ballots[id]===true;return <button key={id} disabled={busy||(!m?.alive&&m?.ghost_vote_used&&!yes)} aria-pressed={yes} className={`${button} ${yes?'bg-violet-400 text-black':'bg-white/5'}`} onClick={()=>{setButlerChecked(false);void run('vote_record',{vote_id:active.id,user_id:id,yes:!yes});}}>{name(id)} · {yes?'O 찬성':'기권'}{!m?.alive&&<span className="block text-xs">{m?.ghost_vote_used?'투표권 없음':'마지막 한 표'}</span>}</button>;})}</div><p>찬성 {Object.values(active.ballots).filter(Boolean).length}표 · 필요 {active.threshold}표</p>{members.filter(m=>m.alive&&m.actual_role==='집사').map(m=><p key={m.user_id} className="text-amber-200">{m.name}의 주인: {members.find(x=>x.user_id===state.engine?.masters?.[m.user_id])?.name??'미지정'} · 건강한 집사는 주인이 손을 들었거나 먼저 집계된 경우에만 찬성할 수 있습니다.</p>)}<label className="flex gap-2"><input type="checkbox" checked={butlerChecked} onChange={e=>setButlerChecked(e.target.checked)}/>집사 등 현장 투표 조건 확인</label><button disabled={busy} className={button} onClick={()=>{if(confirm('현재 표시된 찬성표로 확정할까요? 사망자의 투표권이 차감됩니다.'))void run('vote_finish',{vote_id:active.id,butler_checked:butlerChecked});}}>집계 완료</button></>}</div>}

...
export default function ClocktowerVoting({state,run,busy,error}:{state:LiveState;run:(action:string,data?:Record<string,unknown>)=>Promise<boolean>;busy:boolean;error:string}){
 ...
 return <section className="space-y-4 rounded-3xl border border-violet-400/30 p-5">
  ...
  {state.is_host&&!active&&state.room.day_activity==='VOTING'&&<button disabled={busy} className={button} onClick={()=>void run('day_activity',{activity:'DISCUSSION'})}>토론 재개 안내</button>}
  ...
  {!active&&<form className="space-y-3" onSubmit={async e=>{e.preventDefault();if(await run('vote_nominate',{nominator:mine,nominee})){setNominee('');setNominator('');}}}>
  ...
  {active&&<div className="space-y-3 rounded-2xl bg-violet-400/10 p-4"><p className="text-lg">{name(active.nominator)} → <strong>{name(active.nominee)}</strong> 지목</p>
   ...
   {state.is_host&&<button disabled={busy} className={`${button} text-sm`} onClick={()=>{if(confirm('현재 지목·투표를 취소할까요? 오늘 사용한 지목과 이미 쓴 사망자 투표권은 되돌리지 않습니다. 성결자 등 특수 처리는 이후 수동으로 기록하세요.'))void run('vote_cancel',{vote_id:active.id});}}>특수 판정·투표 취소</button>}
  </div>}
  ...
  <p className="text-xs text-zinc-500">집계만 진행하며 처형·사망은 이야기꾼이 기록합니다. 자유 배치의 자리 번호가 실제 시계방향과 일치하는지 시작 전에 확인하세요.</p>

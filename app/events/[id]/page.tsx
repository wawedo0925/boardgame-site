...
import { CLOCKTOWER_EVENT_DESCRIPTION_PRESET, HOLDEM_EVENT_DESCRIPTION_PRESET } from "@/lib/events/guide";
...
export default function EventDetailPage() {
  ...
  const participationFee = event?.participation_fee ?? (event?.event_kind === "MURDER_MYSTERY" ? 13000 : ["BOARDGAME", "CLOCKTOWER", "HOLDEM"].includes(event?.event_kind ?? "") ? 10000 : 0);
              ...
              <div className="mt-8 flex flex-col gap-8 lg:flex-row lg:items-end lg:justify-between">
                <div>
                  <div className="flex flex-wrap items-center gap-3">
                    <span className={`rounded-full px-3 py-1 text-xs font-semibold ${event.event_kind === "HOLDEM" ? "bg-emerald-400/10 text-emerald-300" : event.event_kind === "MURDER_MYSTERY" ? "bg-red-400/10 text-red-300" : event.event_kind === "CLOCKTOWER" ? "bg-violet-400/10 text-violet-300" : event.event_kind === "GENERAL" ? "bg-sky-400/10 text-sky-300" : "bg-amber-400/10 text-amber-300"}`}>
                      {event.event_kind === "MURDER_MYSTERY"
                        ? "머더미스터리 이벤트"
                        : event.event_kind === "HOLDEM" ? "홀덤"
          ...
          <section className="mx-auto grid max-w-7xl gap-8 px-6 pb-8 pt-14 lg:grid-cols-[1fr_360px]">
            <div className="space-y-8">
              ...
              <article className="rounded-3xl border border-white/10 bg-white/[0.03] p-6 sm:p-8">
                ...
                {guideOpen && event.event_kind === "HOLDEM" ? <div className="mt-6 whitespace-pre-line rounded-2xl border border-emerald-400/20 bg-emerald-400/[0.04] px-5 py-5 text-sm leading-7 text-zinc-300 sm:text-base sm:leading-8">{event.description?.trim() || HOLDEM_EVENT_DESCRIPTION_PRESET}</div> : guideOpen && event.event_kind === "CLOCKTOWER" ? (
                  ...
                  <div className="mt-6 rounded-2xl border border-white/10 bg-white/[0.02] px-4 py-4 text-sm leading-6 text-zinc-400">
                    {event.event_kind === "MURDER_MYSTERY"
                      ...
                      : event.event_kind === "HOLDEM" ? "🃏 텍사스 홀덤만 플레이 · 참가비와 입금 · 늦참 안내를 확인해 주세요."

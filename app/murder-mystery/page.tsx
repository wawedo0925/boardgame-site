...
export default async function MurderMysteryPage() {
  ...
  const [{ data, error }, adminResult, mainAdminResult] = await Promise.all([
  ...
  return <main className="min-h-screen bg-zinc-950 text-white">
    ...
    <section className="mx-auto max-w-7xl px-6 py-16">
      {error ? <div className="rounded-3xl border border-red-500/20 bg-red-500/10 p-6 text-red-200">머더미스터리 목록을 불러오지 못했습니다. {error.message}</div> : <MurderMysteryList mysteries={(data ?? []) as MurderMystery[]} isAdmin={isAdmin} />}

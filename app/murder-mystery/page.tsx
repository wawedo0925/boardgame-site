...
export type MurderMystery = { id:string; title:string; is_unowned:boolean; cover_url:string|null; cover_path:string|null; min_players:number|null; max_players:number|null; play_time:number|null; difficulty:number|null; host_required:boolean; host_requirement:"REQUIRED"|"RECOMMENDED"|"NOT_REQUIRED"; replayable:boolean; theme:string|null; synopsis:string|null };

export default async function MurderMysteryPage() {
  const supabase = await createClient();
  const { data: authData } = await supabase.auth.getUser();
  const userId = authData.user?.id;
  const [{ data, error }, mainAdminResult, interestResult] = await Promise.all([
    supabase.from("murder_mysteries").select("id, title, is_unowned, cover_url, cover_path, min_players, max_players, play_time, difficulty, host_required, host_requirement, replayable, theme, synopsis").order("title", { ascending:true }),
    userId ? supabase.rpc("current_site_role") : Promise.resolve({ data:false }),
    supabase.from("murder_mystery_interests").select("murder_mystery_id"),
  ]);
  const isAdmin = mainAdminResult.data === "MAIN_ADMIN";
  const canRegister = ["MAIN_ADMIN", "ADMIN", "RULE_MASTER"].includes(String(mainAdminResult.data));
  ...
  return <main className="min-h-screen bg-zinc-950 text-white">
    <section className="border-b border-white/10"><div className="mx-auto flex max-w-7xl flex-col gap-6 px-6 py-20 sm:flex-row sm:items-end sm:justify-between">
      ...
      {canRegister && <Link href="/admin/library" className="inline-flex shrink-0 items-center justify-center rounded-xl border border-violet-400/40 bg-violet-400/10 px-4 py-2.5 text-sm font-bold text-violet-200 transition hover:bg-violet-400/20">게임 등록·관리</Link>}
    </div></section>

...
begin
  ...
  select coalesce(nullif(trim(activity_name), ''), '보드라운지 회원')
    into display_name
...
revoke all on function public.save_event_play_review(uuid, integer, text)
  from public, anon;
grant execute on function public.save_event_play_review(uuid, integer, text)
  to authenticated;

-- Allow site/event operators and the assigned group rule master to manage
-- rounds and round-player results. Regular members keep read-only access.
  ...
  select
    exists (
      ...
      where session.id = p_session_id
        and public.can_operate_event(session.event_id)
    )
    or (
...
revoke all on function public.can_manage_event_round(uuid, uuid)
from public, anon;
grant execute on function public.can_manage_event_round(uuid, uuid)
to authenticated;

revoke all on function public.can_manage_event_round_player(uuid)
from public, anon;
grant execute on function public.can_manage_event_round_player(uuid)
to authenticated;

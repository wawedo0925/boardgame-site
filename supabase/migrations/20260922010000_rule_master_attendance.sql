-- Grant rule masters attendance management only; other event permissions stay unchanged.
begin;
...
begin
  ...
  if not (coalesce(public.can_operate_event(p_event_id), false)
          or coalesce(public.current_site_role() = 'RULE_MASTER', false)) then
    raise exception '출석을 관리할 권한이 없습니다.';
  end if;

  if p_status is null or p_status not in ('REGISTERED', 'PRESENT', 'ABSENT') then

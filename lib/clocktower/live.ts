...
export type LiveRequest = { private_once?: boolean; id: string; user_id: string; night: number; prompt: string; target_count: number; allow_self: boolean; status: 'OPEN' | 'SUBMITTED' | 'RESOLVED' | 'CANCELLED'; targets: string[]; result: string; acknowledged: boolean };
export type LiveMission={id:string;round:number;kind:'NUMBERS'|'TEXT'|'SELECT'|'NOTICE';challenge:{tiles?:number[];text?:string;delay_seconds?:number;target_count?:number;superseded?:boolean};completed:boolean};
export type LiveShot={id:string;actor:string;target:string;status:"PENDING"|"DONE";killed:boolean};
export type LiveState = { shots?:LiveShot[]; votes?:LiveVote[];server_now?:string;clock_offset?:number;ghost_vote_used?:boolean; missions?:LiveMission[]; mission_progress?:{user_id:string;completed:number;total:number}[];engine?: NightEngine; engine_version?: number; room: { result_round_id?:string|null;result_saved_at?:string|null;roles_released?:boolean;day_stage?:'PRIVATE'|'NOMINATIONS';flow_revision?:number;winner?:'GOOD'|'EVIL'|null;end_reason?:string|null; id: string; phase: 'SETUP' | 'NIGHT' | 'DAY' | 'ENDED'; night: number; day_activity?:'DISCUSSION'|'VOTING';activity_revision?:number;seating_layout?: SeatLayout; seating_revision?: number } | null; can_create?: boolean; waiting?: boolean; is_host?: boolean; my_id?: string; members?: LiveMember[]; candidates?: { user_id: string; name: string; birth_year?: string | null }[]; requests?: LiveRequest[] };
